...
class ServiceDeployment(ComponentResource):
    ...
    def __init__(
        ...
        service_port_names: Mapping[int, str] = None,
        extra_containers: Sequence[ContainerArgs] = None,
        service_annotations: Mapping[str, str] = None,
        ...
        self.service = Service(
            ...
            spec=ServiceSpecArgs(
                ports=(
                    [
                        ServicePortArgs(
                            name=service_port_names.get(p) if service_port_names else None,
                            port=p,
                            target_port=p,
                        )
                        for p in ports
        ...
        if allocate_ip_address:
            if is_minikube:
                ...
                self.ip_address = self.service.status.apply(load_balancer_address)
        self.register_outputs({})


def load_balancer_address(status) -> str:
